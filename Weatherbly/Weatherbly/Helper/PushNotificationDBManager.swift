...
public final class PushNotificationDBManager {
    ...
    public func readAllNotifications() -> [PushNotification] {
        ...
        if sqlite3_prepare_v2(db, query, -1, &statement, nil) == SQLITE_OK {
            while sqlite3_step(statement) == SQLITE_ROW {
                ...
                let title = columnTextToString(statement: statement, row: 1)
                let message = columnTextToString(statement: statement, row: 2)
                let date = columnTextToString(statement: statement, row: 3)
    ...
    /// 알림 추가
    public func addNotification(title: String, message: String, date: String) {
        let query = "INSERT INTO notifications (title, message, date) VALUES (?, ?, ?);"
        var statement: OpaquePointer?
        var log = ""
