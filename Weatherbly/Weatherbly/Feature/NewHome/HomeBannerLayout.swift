...
public final class HomeBannerLayout: UICollectionViewLayout {
    ...
    public override func prepare() {
        ...
        for section in 0..<collectionView.numberOfSections {
            if section == 0 {
                ...
                attributes.frame = CGRect(x: 0, y: 164, width: Constants.screenWidth - 40, height: 56)
                cache.append(attributes)
                
                // 셀 부분
                let columnWidth = (contentWidth - itemSpacing) / CGFloat(numberOfColumns)
                ...
                for column in 1..<numberOfColumns {
                    ...
                    let offset = (column * columnWidth) + (column * 19)
    ...
    private func getStickyAttributes(at indexPath: IndexPath?) -> UICollectionViewLayoutAttributes? {
        ...
        guard let collectionView,
              let indexPath,
              let stickyAttributes = layoutAttributesForSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, at: indexPath)?.copy() as? UICollectionViewLayoutAttributes
              else {
            return nil
        }
        
        // 스크롤시 collectionView Offset Y 값을 header의 Y값과 비교하여 넘을시 상단 고정 노출
