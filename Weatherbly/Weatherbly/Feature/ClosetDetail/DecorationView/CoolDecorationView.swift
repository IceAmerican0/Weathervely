//
//  CoolDecorationView.swift
//  Weatherbly
//
//  Created by 최수훈 on 6/24/24.
//

import UIKit

final class CoolDecorationView: UICollectionReusableView {
    
    private let decoWrapper = UIView()
    private let bgImage = UIImageView().then {
        $0.contentMode = .scaleAspectFill
        $0.image = UIImage.moreCool_banner
    }
    private let tempIconImage = UIImageView().then {
        $0.contentMode = .scaleAspectFit
        $0.image = UIImage.moreCool_illust
    }
